...
class TracePreprocessor:
    def __init__(self, csv_path):
        ...
        self.attr_types = []  # Almacenará 'cat' para categóricos, 'time' para fechas
        self.vocabs = []      
        self.stoi = []        
        self.itos = []        
        self.dims = []        
        
        # Estructuras para manejar el tiempo relativo
        self.log_delta_mins = {}
        self.log_delta_maxs = {}
    ...
    def load_and_process(self):
        ...
        # --- PASO 1: Descubrimiento y Extracción ---
        for r_idx, row in enumerate(raw_rows):
            ...
            for t, item in enumerate(row):
                ...
                # Procesamiento y cálculo de Deltas
                if len(parts) == self.num_attributes:
                    ...
                    for i, val in enumerate(parts):
                        if self.attr_types[i] == 'cat':
                            ...
                            # Logaritmo para estabilizar varianzas de tiempo extremas
        ...
        # --- PASO 2: Construcción de Vocabularios ---
        ...
        for i, row in enumerate(parsed_data):
            ...
            # Padding <PAD> para que todas las trazas tengan la misma longuitud
    ...
    def decode_traces(self, generated_data):
        ...
        decoded_traces = []
        base_start_time = self.trace_start_times[0] if self.trace_start_times else datetime.now().timestamp()
        
        for i in range(len(generated_data)):
            trace = []
            # Reloj individual para cada traza generada
            ...
            for t in range(len(generated_data[i])):
                ...
                for attr_idx in range(self.num_attributes):
                    ...
                    if self.attr_types[attr_idx] == 'cat':
                        ...
                        # Clip en 0.0 asegura la cronología estricta (no hay deltas negativos)
    ...
    def train(self, dataset, epochs_f1=300, epochs_f2=300, epochs_f3=300, loss_log_path=None):
        ...
        for epoch in range(epochs_f3):
            for X_batch in dataloader:
                ...
                loss_gen = loss_g_u + 10 * loss_g_s + 10 * loss_v
                loss_gen.backward()
                self.g_opt.step()

                # B. ENTRENAR DISCRIMINADOR
...
if __name__ == "__main__":
    ...
    # Hiperparámetros de la Red (Ahora usan nargs='+' para aceptar listas)
    ...
    # Learning Rates (Ahora usan nargs='+' para aceptar listas)
